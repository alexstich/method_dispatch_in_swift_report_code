...
// cat
sil_global hidden [let] @vtables_d.cat : vtables_d.Cat : $Cat

// main
sil @main : $@convention(c) (Int32, UnsafeMutablePointer<Optional<UnsafeMutablePointer<Int8>>>) -> Int32 {
bb0(%0 : $Int32, %1 : $UnsafeMutablePointer<Optional<UnsafeMutablePointer<Int8>>>):
  alloc_global @vtables_d.cat : vtables_d.Cat         // id: %2
  %3 = global_addr @vtables_d.cat : vtables_d.Cat : $*Cat // users: %8, %7
  %4 = metatype $@thick Cat.Type                  // user: %6
  // function_ref Cat.__allocating_init()
  %5 = function_ref @vtables_d.Cat.__allocating_init() -> vtables_d.Cat : $@convention(method) (@thick Cat.Type) -> @owned Cat // user: %6
  %6 = apply %5(%4) : $@convention(method) (@thick Cat.Type) -> @owned Cat // user: %7
  store %6 to %3 : $*Cat                          // id: %7
  %8 = load %3 : $*Cat                            // user: %9
  %9 = upcast %8 : $Cat to $Animal                // user: %11
...
// Animal.sleep()
sil hidden @vtables_d.Animal.sleep() -> () : $@convention(method) (@guaranteed Animal) -> () {
// %0 "self"                                      // user: %1
bb0(%0 : $Animal):
  ...
  %27 = tuple ()                                  // user: %28
  return %27 : $()                                // id: %28
} // end sil function 'vtables_d.Animal.sleep() -> ()'
...
// Animal.go()
sil hidden @vtables_d.Animal.go() -> () : $@convention(method) (@guaranteed Animal) -> () {
// %0 "self"                                      // user: %1
bb0(%0 : $Animal):
  debug_value %0 : $Animal, let, name "self", argno 1, implicit // id: %1
  %2 = integer_literal $Builtin.Word, 1           // user: %4
  ...
  %27 = tuple ()                                  // user: %28
  return %27 : $()                                // id: %28
} // end sil function 'vtables_d.Animal.go() -> ()'

// Animal.deinit
sil hidden @vtables_d.Animal.deinit : $@convention(method) (@guaranteed Animal) -> @owned Builtin.NativeObject {
// %0 "self"                                      // users: %2, %1
bb0(%0 : $Animal):
  debug_value %0 : $Animal, let, name "self", argno 1, implicit // id: %1
  %2 = unchecked_ref_cast %0 : $Animal to $Builtin.NativeObject // user: %3
  return %2 : $Builtin.NativeObject               // id: %3
} // end sil function 'vtables_d.Animal.deinit'

// Animal.__deallocating_deinit
sil hidden @vtables_d.Animal.__deallocating_deinit : $@convention(method) (@owned Animal) -> () {
// %0 "self"                                      // users: %3, %1
bb0(%0 : $Animal):
  debug_value %0 : $Animal, let, name "self", argno 1, implicit // id: %1
  // function_ref Animal.deinit
  %2 = function_ref @vtables_d.Animal.deinit : $@convention(method) (@guaranteed Animal) -> @owned Builtin.NativeObject // user: %3
  ...
  %6 = tuple ()                                   // user: %7
  return %6 : $()                                 // id: %7
} // end sil function 'vtables_d.Animal.__deallocating_deinit'

// Animal.__allocating_init()
sil hidden [exact_self_class] @vtables_d.Animal.__allocating_init() -> vtables_d.Animal : $@convention(method) (@thick Animal.Type) -> @owned Animal {
// %0 "$metatype"
bb0(%0 : $@thick Animal.Type):
  %1 = alloc_ref $Animal                          // user: %3
  // function_ref Animal.init()
  %2 = function_ref @vtables_d.Animal.init() -> vtables_d.Animal : $@convention(method) (@owned Animal) -> @owned Animal // user: %3
  %3 = apply %2(%1) : $@convention(method) (@owned Animal) -> @owned Animal // user: %4
  return %3 : $Animal                             // id: %4
} // end sil function 'vtables_d.Animal.__allocating_init() -> vtables_d.Animal'

// Animal.init()
sil hidden @vtables_d.Animal.init() -> vtables_d.Animal : $@convention(method) (@owned Animal) -> @owned Animal {
// %0 "self"                                      // users: %2, %1
bb0(%0 : $Animal):
  debug_value %0 : $Animal, let, name "self", argno 1, implicit // id: %1
  return %0 : $Animal                             // id: %2
} // end sil function 'vtables_d.Animal.init() -> vtables_d.Animal'

// Cat.go()
sil hidden @vtables_d.Cat.go() -> () : $@convention(method) (@guaranteed Cat) -> () {
// %0 "self"                                      // user: %1
bb0(%0 : $Cat):
  ...
  %27 = tuple ()                                  // user: %28
  return %27 : $()                                // id: %28
} // end sil function 'vtables_d.Cat.go() -> ()'

// Cat.__allocating_init()
sil hidden [exact_self_class] @vtables_d.Cat.__allocating_init() -> vtables_d.Cat : $@convention(method) (@thick Cat.Type) -> @owned Cat {
// %0 "$metatype"
bb0(%0 : $@thick Cat.Type):
  %1 = alloc_ref $Cat                             // user: %3
  // function_ref Cat.init()
  %2 = function_ref @vtables_d.Cat.init() -> vtables_d.Cat : $@convention(method) (@owned Cat) -> @owned Cat // user: %3
  %3 = apply %2(%1) : $@convention(method) (@owned Cat) -> @owned Cat // user: %4
  return %3 : $Cat                                // id: %4
} // end sil function 'vtables_d.Cat.__allocating_init() -> vtables_d.Cat'

// Cat.init()
sil hidden @vtables_d.Cat.init() -> vtables_d.Cat : $@convention(method) (@owned Cat) -> @owned Cat {
// %0 "self"                                      // user: %2
bb0(%0 : $Cat):
  ...
  // function_ref Animal.init()
  %5 = function_ref @vtables_d.Animal.init() -> vtables_d.Animal : $@convention(method) (@owned Animal) -> @owned Animal // user: %6
  ...
  return %7 : $Cat                                // id: %12
} // end sil function 'vtables_d.Cat.init() -> vtables_d.Cat'

// Cat.deinit
sil hidden @vtables_d.Cat.deinit : $@convention(method) (@guaranteed Cat) -> @owned Builtin.NativeObject {
// %0 "self"                                      // users: %2, %1
bb0(%0 : $Cat):
  debug_value %0 : $Cat, let, name "self", argno 1, implicit // id: %1
  %2 = upcast %0 : $Cat to $Animal                // user: %4
  // function_ref Animal.deinit
  %3 = function_ref @vtables_d.Animal.deinit : $@convention(method) (@guaranteed Animal) -> @owned Builtin.NativeObject // user: %4
  %4 = apply %3(%2) : $@convention(method) (@guaranteed Animal) -> @owned Builtin.NativeObject // user: %5
  return %4 : $Builtin.NativeObject               // id: %5
} // end sil function 'vtables_d.Cat.deinit'

// Cat.__deallocating_deinit
sil hidden @vtables_d.Cat.__deallocating_deinit : $@convention(method) (@owned Cat) -> () {
// %0 "self"                                      // users: %3, %1
bb0(%0 : $Cat):
  debug_value %0 : $Cat, let, name "self", argno 1, implicit // id: %1
  // function_ref Cat.deinit
  %2 = function_ref @vtables_d.Cat.deinit : $@convention(method) (@guaranteed Cat) -> @owned Builtin.NativeObject // user: %3
  ...
  %6 = tuple ()                                   // user: %7
  return %6 : $()                                 // id: %7
} // end sil function 'vtables_d.Cat.__deallocating_deinit'
...
sil_vtable Animal {
  ...
  #Animal.init!allocator: (Animal.Type) -> () -> Animal : @vtables_d.Animal.__allocating_init() -> vtables_d.Animal	// Animal.__allocating_init()
  #Animal.deinit!deallocator: @vtables_d.Animal.__deallocating_deinit	// Animal.__deallocating_deinit
}

sil_vtable Cat {
  #Animal.go: (Animal) -> () -> () : @vtables_d.Cat.go() -> () [override]	// Cat.go()
  #Animal.init!allocator: (Animal.Type) -> () -> Animal : @vtables_d.Cat.__allocating_init() -> vtables_d.Cat [override]	// Cat.__allocating_init()
  #Cat.deinit!deallocator: @vtables_d.Cat.__deallocating_deinit	// Cat.__deallocating_deinit
}



// Mappings from '#fileID' to '#filePath':
//   'vtables_d/vtables_d.swift' => 'vtables_d.swift'
